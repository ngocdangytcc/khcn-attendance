...
import { logAttendance } from '../services/attendanceService';     // 👈 THÊM DÒNG NÀY
...
const Dashboard: React.FC<DashboardProps> = ({ onNotification }) => {
  ...
  const handleStartCheck = async (type: 'CHECK_IN' | 'CHECK_OUT') => {
    ...
    try {
        if (!isWifiConnection()) {
          console.warn("Detected non-wifi connection");
        }
        ...
        if (wifiConfigs.length > 0) {
            ...
            const validConfig = wifiConfigs.find(config => config.ip === currentIP);
  ...
  const handleCapture = async (imageSrc: string) => {
    ...
    try {
      ...
      if (result.isMatch) {
        ...
        // 2. LƯU NỘI BỘ LOCAL DEVICE
        saveAttendanceLog(record);

        // 3. LƯU LÊN GOOGLE SHEET
        try {
          await logAttendance({
            employeeId: employee.id,
            employeeName: employee.name,
        ...
        onNotification(
          `Thành công! ${checkType === 'CHECK_IN' ? 'Vào ca' : 'Ra ca'} (${Math.round(result.confidence * 100)}%)`,
          'success'
        );

      } else {
        onNotification(`Thất bại: Khuôn mặt không khớp (${result.reasoning})`, 'error');
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        ...
        {currentEmployee && (
            <div className="mt-4 flex items-center space-x-3 bg-blue-50 p-3 rounded-lg">
                <img src={currentEmployee.avatar} alt="ref" className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm" />
                <div>
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center flex-col space-y-4 px-8 text-center">
             ...
             {statusMessage.includes('Wifi') && (
                 <p className="text-white/60 text-xs">Đang kiểm tra địa chỉ IP mạng...</p>
             )}
        </div>
      )}
      ...
      {showCamera && (
        <Camera
          ...
          instruction={`Vui lòng nhìn thẳng để ${checkType === 'CHECK_IN' ? 'Vào ca' : 'Ra ca'}`}
        />
      )}
    </div>
  );
